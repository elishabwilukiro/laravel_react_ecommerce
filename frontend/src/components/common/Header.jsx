          ...
          <Navbar expand="lg" sticky="top" className="">
            ...
            <Nav
              ...
              >
              {
                categories && categories.map(category=>{
                  return(
                    <Nav.Link href={`/shop?category=${category.id}`} key={category.id}>
                      {category.name}
                    </Nav.Link>
                  )
                }) 
            ...
            <div className="nav-right d-flex">
              ...
              <Link to={`/cart`} className='ms-3'>
                ...
                <span className="badge badge-sm bg-danger text-white rounded-circle">{cartData.length}</span>
              </Link>
            </div>
            </Navbar.Collapse>
          </Navbar>
