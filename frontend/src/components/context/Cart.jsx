...
export const CartProvider = ({children}) => {
     ...
     const addToCart = (product, size=null) => {
          ...
          if(cartData.length == 0){
               ...
               // Size not empty
               if(size != null){
                    const isProductExist = updatedCart.find(item => item && item.product_id == product.id && item.size == size)
                    ...
                    // Size is null
                    const isProductExist = updatedCart.find(item => item && item.product_id == product.id)
     ...
     const updateCartItem = (itemId, newQty) => {
          let updatedCart = [...cartData];
          updatedCart = updatedCart.map(item =>
               (item && item.id == itemId) ? {...item, qty: newQty} : item
          )
          setCartData(updatedCart)
          localStorage.setItem('cart',JSON.stringify(updatedCart))
     }
